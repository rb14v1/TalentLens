...
const HiringManagerSidebar = ({ setCollapsed }) => {
  ...
  const [collapsed, setLocalCollapsed] = useState(true); // default collapse
...
<button
        ...
        className={`flex items-center gap-4 px-4 py-3 w-full text-lg font-medium transition-all duration-200
...
<div
          ...
          style={{ width: collapsed ? 44 : 36, height: collapsed ? 44 : 36 }}
>
<div
...
<>
      ...
      {/* ⭐ FIXED SIDEBAR BELOW GLOBAL HEADER (72px) */}
<aside
...
>
          ...
          {!collapsed && (
<h3 className="uppercase text-teal-200 font-semibold tracking-wide text-xl">
...
<div className="px-4 flex-1 space-y-3">
  <SidebarBtn label="Manual JD Create" icon={<FileEdit />} link="/description" />
  <SidebarBtn label="Draft" icon={<FileClock />} link="/drafts" />
  <SidebarBtn label="Published JDs" icon={<FileCheck />} link="/published-jds" />
  <SidebarBtn label="Matched Resume" icon={<UsersRound />} link="/matchedresume" />
</div>
...
<div
          ...
          className={`p-4 border-t border-white/20 bg-[#0A4C5E]/90 flex items-center gap-3 ${
...
<img
              ...
              className="w-11 h-11 rounded-full border-2 border-white/70 shadow-md object-cover"
 
            />
 
            {/* Pencil when expanded */}
...
<p className="text-xs text-teal-200 truncate">{user.email || "No Email"}</p>
</div>
 
          )}
 
          {/* Logout only in expanded mode */}
