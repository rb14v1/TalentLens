...
const PublishedJDs = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const allJobs = Array.isArray(jobsJson.results) ? jobsJson.results : (Array.isArray(jobsJson) ? jobsJson : []);
  ...
  const handleViewJD = (job) => {
    ...
    // 1. Clean up the title (e.g., "Senior Dev" -> "Senior-Dev")
    const safeTitle = (job.title || "Job").replace(/\s+/g, "-").replace(/[^a-zA-Z0-9-]/g, "");
 
    // 2. ✅ FIX: Open the FRONTEND URL, NOT the API URL
    // This tells React to load your 'ViewJD.jsx' page
  ...
  const handleEdit = (job) => {
    // Navigate to your "Publish/Create" page with the job data
    // You will need to update your PublishJD.jsx to read this "state" and pre-fill the form
  ...
  const JobCard = ({ job, isMine }) => {
    ...
    return (
      <div className={`bg-white rounded-2xl p-5 shadow-sm border transition relative mb-6
        ...
        <div className="flex justify-between items-start mb-3">
            <div className="flex-1 pr-4">
                ...
                <p className="text-gray-500 font-medium text-xs mt-1">{job.department || "General"}</p>
            ...
            <button
                ...
                className={`
                    text-[10px] uppercase tracking-wider font-bold px-2 py-1 rounded-full flex items-center gap-1 transition-all shadow-sm cursor-pointer hover:opacity-80 shrink-0
                    ${isActive
        ...
        {/* ✅ FOOTER ACTIONS: View (Left), Download (Middle), Delete (Right) */}
  ...
  return (
    <div className="min-h-screen flex bg-[#E9F1F4]">
      <div className="w-64 flex-shrink-0"><HiringManagerSidebar /></div>
      <main className="flex-1 py-10 pr-10 pl-20 relative overflow-y-auto">
        <button onClick={() => navigate(-1)} className="absolute top-8 right-8 bg-white text-[#0F394D] px-5 py-2 rounded-full shadow hover:bg-gray-50">Back</button>
 
        <h1 className="text-3xl font-bold text-[#0D1F29] mb-8">Published Job Descriptions</h1>
 
        {loading ? <div className="flex justify-center h-64"><Loader className="animate-spin text-[#21B0BE]" size={40}/></div> : (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                <div>
                    <h2 className="text-xl font-bold text-[#0F394D] mb-4 border-b-2 border-[#21B0BE] pb-2 flex justify-between">
                        My Job Postings <span className="bg-[#21B0BE]/10 text-[#0F394D] text-xs px-2 py-1 rounded-full">{myJobs.length}</span>
                    </h2>
                    {myJobs.length > 0 ? (
                        myJobs.map(job => <JobCard key={job.id} job={job} isMine={true} />)
                    ) : (
                        <p className="text-gray-400 italic mt-4 p-4 border border-dashed rounded-lg bg-white">No jobs found for you.</p>
                    )}
                </div>
                <div>
                    <h2 className="text-xl font-bold text-[#0F394D] mb-4 border-b-2 border-gray-300 pb-2 flex justify-between">
                        Department Postings <span className="bg-gray-200 text-gray-600 text-xs px-2 py-1 rounded-full">{deptJobs.length}</span>
                    </h2>
                    {deptJobs.length > 0 ? (
                        deptJobs.map(job => <JobCard key={job.id} job={job} isMine={false} />)
                    ) : (
                        <p className="text-gray-400 italic mt-4 p-4 border border-dashed rounded-lg bg-white">No other jobs in this department.</p>
                    )}
                </div>
            </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 transform transition-all scale-100">
             <div className="flex flex-col items-center text-center">
                ...
                <h3 className="text-2xl font-bold text-[#0F394D] mb-2">Delete Job Description?</h3>
