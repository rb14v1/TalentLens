...
const formatDate = (dateString) => {
  // 1. If completely missing/null
  if (!dateString) return "N/A"; 
  
  // 2. If backend specifically says "No Date"
  if (dateString === "No Date") return "No Date (DB)";

  try {
    const date = new Date(dateString);
    // 3. If invalid date, return the raw string (don't hide it!)
    if (isNaN(date.getTime())) return dateString; 
    
    // 4. Success! Format nicely
...
const PublishedJDs = () => {
  ...
  const handleViewJD = (job) => {
    const fileName = job.file_name || (job.s3_url ? job.s3_url.split('/').pop() : null);
    if (!fileName) return alert("No PDF file found");
    window.open(`${API_BASE_URL}/view_resume/?file_name=${encodeURIComponent(fileName)}`, "_blank");
  };
  ...
  const JobCard = ({ job, isMine }) => {
    ...
    // Use our helper to get the date string
    const displayDate = formatDate(job.created_at);

    return (
      <div className={`bg-white rounded-2xl p-6 shadow-sm border transition relative mb-6 
        ...
        <div className="flex justify-between items-start mb-2">
            <div className="flex-1">
                <div className="flex items-center gap-3 flex-wrap mb-1">
                    <h3 className={`text-xl font-bold ${isActive ? "text-[#0F394D]" : "text-gray-500"}`}>
                        {job.title || "Untitled"}
                    </h3>

                    {/* ✅ DISPLAY DATE (Formatted or Raw) */}
                ...
                <p className="text-gray-500 font-medium text-sm">{job.department || "General"}</p>
        ...
        <div className="flex items-center gap-3 text-sm text-gray-600 mb-5 mt-3">
          ...
          <span className="bg-gray-50 px-2.5 py-1 rounded-md border border-gray-100 flex gap-1.5 items-center text-xs font-medium">
            <Briefcase size={14} className="text-gray-400"/> {job.type || "Full-time"}
          </span>
        </div>
